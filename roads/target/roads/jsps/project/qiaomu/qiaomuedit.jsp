...
<head>
    ...
    <script type="text/javascript">
        function editeItemsListSubmit() {
            document.itemsform.action="<c:url value="/qiaomu/editQiaoMusSubmit.action"/>";
...
<form name="itemsform" onsubmit="return dosubmit()" method="post" action="<c:url value="/qiaomu/editQiaoMusSubmit.action"/>">
<table width="100%" border=1>
    ...
    <c:forEach items="${itemsList}" var="item" varStatus="Status">
        <tr>
            <input type="hidden" name="itemsList[${Status.index}].ls_id" value="${item.ls_id}"/>
