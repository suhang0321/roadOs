...
<head>
    ...
    <script type="text/javascript">
        function editeItemsListSubmit() {
            document.itemsform.action="<c:url value="/yundong/editYunDongsSubmit.action"/>";
...
<form name="itemsform"  onsubmit="return dosubmit()" method="post"action="<c:url value="/yundong/editYunDongsSubmit.action"/>">
<table width="100%" border=1>
    ...
    <c:forEach items="${itemsList}" var="item" varStatus="Status">
        <tr>
            ...
            <td><input type="text" name="itemsList[${Status.index}].yd_unit" value="${item.yd_unit}"/></td>
            <td><input type="text" name="itemsList[${Status.index}].yd_location" value="${item.yd_location}"/></td>
            <td><input type="text" name="itemsList[${Status.index}].yd_type" value="${item.yd_type}"/></td>
